...
| `/vfx playat <эффект> <x> <y> <z> [игроки]` | Запустить эффект с переопределением `pos_x/y/z` (удобно для `block_tint`/`block_outline`). |
| `/vfx stop <эффект> [игроки]` | Остановить эффект. Эффекты с `fade_ticks > 0` плавно гаснут. |
| `/vfx set <эффект> <параметр> <значение> [игроки]` | Живой override параметра **играющего** эффекта, без перезапуска таймлайна. Эффект должен быть запущен (иначе команда игнорируется клиентом). |
