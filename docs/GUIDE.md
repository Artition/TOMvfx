...
| `/vfx playat <эффект> <x> <y> <z> [игроки]` | Запустить эффект с переопределением `pos_x/y/z` (для `block_tint`/`block_outline`) **и** перепривязкой мировых биндов (`screen_x/y`, `proximity`) к этой точке. |
| `/vfx stop <эффект> [игроки]` | Остановить эффект (все его экземпляры). Эффекты с `fade_ticks > 0` плавно гаснут. |
| `/vfx set <эффект> "<параметры>" [игроки]` | Живой override параметров **играющего** эффекта, без перезапуска таймлайна. Параметры — строка в кавычках: `"radius:8"` или `"radius:8,strength:0.5"` (Tab подсказывает `имя:`). Если эффект не играет — запускается persistent-экземпляр с этими значениями. |
| `/vfx key <эффект> <параметр> <время> <значение> [easing] [игроки]` | Добавить/заменить ключевой кадр параметра играющего эффекта (время — тики от старта, easing — кривая к следующему кадру, Tab-автодополнение). |
