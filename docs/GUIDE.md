...
| `/vfx playentity <effect> [{[...]}] <targets>` | Play an effect on selected entities (selector, e.g. `@e[type=!player,distance=..10]`). The effect is sent to the executing player; targets are passed by UUID (up to 16) and apply to `entity_tint`/`entity_outline`. The optional param-map — overrides. |
