...
| `/vfx set <эффект> {[параметр:значение],...} [игроки]` | Живой override параметров **играющего** эффекта, без перезапуска таймлайна. Если эффект не играет — запускается persistent-экземпляр с этими значениями. Синтаксис подсказывается по Tab: `{` → `[` → имя параметра → `:` значение → `]` → `,` или `}`. |
