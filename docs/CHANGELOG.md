...
## [Unreleased]
### Added
- Параметр `target` для `entity_tint`/`entity_outline` в `/vfx set`:live-retarget без перезапуска; Tab-подсказка выдаёт UUID моба под прицелом (рейкаст 64 блока). Сетевой action `SET_TARGET` (protocol v4).
- `/vfx set` принимает несколько параметров сразу: синтаксис `{[имя:значение],...}` с живыми Tab-подсказками на каждом шаге (`{`, `[`, имя, `]`, `,`/`}`); при отсутствии играющего экземпляра запускает persistent-экземпляр с этими значениями.
- `/vfx playat` перепривязывает мировые бинды (`screen_x/y`, `proximity`) к указанной точке.
- Пост-эффекты `invert` (инверсия цветов) и `vortex` (закручивание пикселей вокруг точки, aspect-correct, отрицательный `strength` меняет направление).
- Несколько экземпляров одного эффекта одновременно: повторный `/vfx play` добавляет независимый экземпляр вместо замены; `/vfx stop` останавливает все экземпляры id.
