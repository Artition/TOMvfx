...
## [Unreleased]
### Added
- **`gradient_map`: режим построения градиента и координата цвета.** Новые параметры `mode` (0 = linear, 1 = constant/stepped) и `pos` (0..1, координата цвета). В linear-режиме `pos` сдвигает центр перехода (0.5 — без сдвига); в stepped-режиме — жёсткий порог: ярче `pos` → цвет `to`, темнее → `from` (удобно для масок/стилизованных теней). У встроенного `tompfx:gradient_map` добавлены дефолты `mode: 0`, `pos: 0.5`.
