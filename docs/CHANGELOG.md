...
## [Unreleased]
### Added
- `/vfx set` принимает несколько параметров сразу: строка `"имя:значение,имя:значение"` с Tab-подсказками имён; при отсутствии играющего экземпляра запускает persistent-экземпляр с этими значениями.
