...
## [Unreleased]
### Fixed
- **`cinematic_camera`: исправлена прорисовка и "отскок" камеры.** Сглаживание/тряска теперь применяются в `Camera.update` ДО построения cull-фрустума и вью-матрицы (инжект в HEAD `prepareCullFrustum` вместо TAIL `update`), поэтому фрустум отсечения, вью-матрица и ориентация камеры всегда согласованы — мир больше не "уезжает"/не отсекается неверно. Yaw отслеживается в непрерывном (unwrapped) домене через кратчайшую покадровую дельту, так что при большом лаге сглаживания камера не переворачивается в обратную сторону на быстрых поворотах.
### Added
