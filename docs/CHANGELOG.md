...
## [Unreleased]
### Added
- `/vfx set` принимает несколько параметров сразу: синтаксис `{[имя:значение],...}` с пошаговыми Tab-подсказками; при отсутствии играющего экземпляра запускает persistent-экземпляр с этими значениями.
