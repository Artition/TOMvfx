...
public final class VFXPostProcessingManager {
	private static final Logger LOGGER = LoggerFactory.getLogger("tompfx/post");
	...
	public void process(final VFXEffectManager effects, final RenderTarget mainTarget) {
		...
		if (this.projectionMatrixBuffer == null) {
			this.projectionMatrixBuffer = new ProjectionMatrixBuffer("tompfx_post");
		}
	...
	private void ensureTargets(final int width, final int height) {
		if (width != this.lastWidth || height != this.lastHeight) {
			for (int i = 0; i < this.pingPong.length; i++) {
				...
				this.pingPong[i] = new TextureTarget("tompfx pingpong " + i, width, height, false);
			}
